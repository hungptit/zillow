...
classdef HouseFactTable < handle
    ...
    methods (Access = public)
        ...
        function write(this, data, updateInfo)
            ...
            results = this.DBConnection.exec(sprintf('SELECT * FROM %s WHERE cityid=? AND street=?', ...
                                                     zillow.Utilities.FactTableName), ...
                                             cityId, data.Street);
            if isempty(results)
                cmd = sprintf(['INSERT INTO %s (zpid, link, street, ' ...
                               'cityid, zipcode, latitude, longitude, ' ...
                               'usecode, yearbuilt, lotsizesqft, ' ...
                               'finishedsqft, bathrooms, bedrooms, ' ...
                               'totalrooms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'], ...
                              zillow.Utilities.FactTableName);
                this.DBConnection.exec(cmd, data.zpid, data.Link, data.Street, cityId, ...
                                       data.ZipCode, data.Latitude, data.Longitude, ...
                                       data.UseCode, data.YearBuilt, data.LotSizeSqFt, data.FinishedSqFt, ...
                                       data.Bathrooms, data.Bedrooms, data.TotalRooms);
            ...
            if ~isempty(updateInfo)
                infoCmd = sprintf(['INSERT INTO %s (zpid, taxassessment, ' ...
                                   'taxassessmentyear, lastsolddate, ' ...
                                   'lastsoldprice, zestimate, ' ...
                                   'homedescriptions, ' ...
                                   'pageviewcountthismonth, ' ...
                                   'pageviewcounttotal, querydate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'], ...
                ...
                infoCmd = sprintf(['INSERT INTO %s (zpid, taxassessment, ' ...
                                   'taxassessmentyear, lastsolddate, ' ...
                                   'lastsoldprice, zestimate, querydate) VALUES (?, ?, ?, ?, ?, ?, ?)'], ...
                                  zillow.Utilities.InfoTableName);
                this.DBConnection.exec(infoCmd, data.zpid, data.TaxAssessment, data.TaxAssessmentYear, ...
                                       datestr(data.LastSoldDate, 'yyyy/mm/dd'), ...
                                       data.LastSoldPrice, data.ZEstimate, ...
    ...
    methods (Access = public)
        ...
        function id = getCityId(this, cityName, stateName)
            ...
            results = this.Cities(arrayfun(@(aCity) ((aCity.sid == stateId) && strcmpi(aCity.name, cityName)), ...
                                           this.Cities));
        ...
        function createStateTable(this)
            ...
            this.DBConnection.exec(['CREATE TABLE IF NOT EXISTS ' ...
                                'States (ID INTEGER PRIMARY KEY ' ...
                                'AUTOINCREMENT, Name varchar(64) ' ...
                                'NOT NULL UNIQUE, ShortName char(2) NOT NULL UNIQUE)']);
        ...
        function createCityTable(this)
            ...
            this.DBConnection.exec(['CREATE TABLE  IF NOT EXISTS ' ...
                                ...
                                'AUTOINCREMENT, SID INTEGER, Name varchar(128))']);
